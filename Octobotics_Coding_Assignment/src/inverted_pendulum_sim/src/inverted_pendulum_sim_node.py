...
class InvertedPendulum:
    ...
    def cart(self):
        ...
        pg.draw.rect(DISPSURFACE, CARTCOLOR, (int(x), int(y), self.cart_w, self.cart_h))
        pg.draw.rect(DISPSURFACE, BLACK, (int(x), int(y), self.cart_w, self.cart_h), 3)

        pg.draw.circle(DISPSURFACE, BLACK, (round(x), round(y + self.cart_h / 2 + 4 * self.wheel_r)), 2 * round(self.wheel_r), 7)
        pg.draw.circle(DISPSURFACE, WHEELCOLOR, (round(x), round(y + self.cart_h / 2 + 4 * self.wheel_r)), 2 * round(self.wheel_r - 3.5))
        pg.draw.circle(DISPSURFACE, BLACK, (round(x + self.cart_w), round(y + self.cart_h / 2 + 4 * self.wheel_r)), 2 * round(self.wheel_r),
                       7)
        pg.draw.circle(DISPSURFACE, WHEELCOLOR, (round(x + self.cart_w), round(y + self.cart_h / 2 + 4 * self.wheel_r)),
                       2 * round((self.wheel_r - 3.5)))
    ...
    def pendulum(self):
        ...
        pg.draw.line(DISPSURFACE, PENDULUMCOLOR, (x, self.y),
                     (round(x + self.length * sin(self.theta)), round(self.y + self.length * cos(self.theta))), 10)

        pg.draw.circle(DISPSURFACE, BLOBCOLOR,
                       (round(x + self.length * sin(self.theta)), round(self.y + self.length * cos(self.theta))), 25)
        pg.draw.circle(DISPSURFACE, BLACK,
                       (round(x + self.length * sin(self.theta)), round(self.y + self.length * cos(self.theta))), 25, 4)

        # draw a circle centered at x, y
        pg.draw.circle(DISPSURFACE, RED, (round(x), round(self.y)), 10)
